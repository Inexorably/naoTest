...
// Member vectors are of size 2*MAX_EXPRESSION_INSTANCES.
struct Expression {
...
// The genetics of an organism.  Returns a value based on the 3 state variables.
struct Genome {
  // One Expression per variable, so size 3 vector.
...
struct Organism {
    ...
    // Holds the genetics of the organism.  We have 3 inputs of interest:
    ...
    // 3 input variables, ie vector is size 8.
    std::vector<Genome> m_genetics;
